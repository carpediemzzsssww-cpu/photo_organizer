...
class PhotoAnalyzer:
    ...
    def extract_features(self, image_path: str) -> Dict:
        ...
        try:
            ...
            # 判断是否为截图(只识别明确的截图关键词,不包括IMG_这种相机默认前缀)
            filename_lower = os.path.basename(image_path).lower()
            features['is_screenshot'] = any(keyword in filename_lower 
                                           for keyword in ['screenshot', '截图', 'screen shot', '屏幕截图', 'screenrecord'])
...
class PhotoOrganizer:
    ...
    def find_similar_groups(self, threshold: int = 15, debug: bool = False):
        """找出相似照片组(threshold越大,检测越宽松)"""
        print(f"🔍 正在检测相似照片(阈值:{threshold})...")
        ...
        for i, photo1 in enumerate(self.photos):
            ...
            for j, photo2 in enumerate(self.photos[i+1:], i+1):
                ...
                if debug:
                    print(f"  [{photo1['filename']}] vs [{photo2['filename']}]: 距离={distance}")
            ...
            if len(group) > 1:
                ...
                if debug:
                    print(f"  ✅ 发现相似组: {[p['filename'] for p in group]}")
        ...
        for group in self.similar_groups:
            ...
            for idx in delete_indices:
                ...
                try:
                    ...
                    continue  # 无法转换则跳过
                if 0 < idx_int <= len(group):
                    deletable_photos.append(group[idx_int-1])
...
"""
            for i, group in enumerate(self.similar_groups, 1):
                decision = group[0].get('ai_decision', {})
                raw_keep_indices = decision.get('recommended_keep', [1])
                # 安全地将 keep_indices 中的元素转换为整数
                keep_indices = []
...
def main():
    ...
    if len(sys.argv) < 2:
        print("❌ 用法: python photo_organizer.py <照片文件夹路径> [--debug]")
        print("📝 示例: python photo_organizer.py ~/Pictures/test_photos")
        print("🐛 调试: python photo_organizer.py ~/Pictures/test_photos --debug")
        sys.exit(1)
    ...
    organizer.find_similar_groups(debug=debug_mode)
